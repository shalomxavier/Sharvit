...
const TradingCollectionsTable3and6: React.FC = () => {
  ...
  useEffect(() => {
    const q = query(
      collection(db, 'trading_collections_3and6'),
  ...
  if (loading) {
    return (
      <div className="bg-white border rounded-lg p-6 shadow-sm">
        <h3 className="text-xl font-bold mb-4">Trading Collections (3&amp;6)</h3>
              ...
              {collections.map((collection) => (
                <tr key={collection.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-2 px-3">
                    <span className={`px-2 py-1 rounded text-xs font-medium ${getStatusBadgeClass(collection.status)}`}>
                      {collection.status}
                    </span>
                  </td>
                    ...
                    {collection.buySignal ? (
                      <div>
                        ...
                        <div className="font-mono">
                          {formatPrice(collection.buySignal.price)}
                        </div>
                      </div>
                    ) : (
                  ...
                  <td className="py-2 px-3 text-sm">
                    {collection.sellSignal ? (
                      <div>
                        <span className={`px-2 py-1 rounded text-xs font-medium ${getSellStatusBadgeClass(collection.sellSignal.status)}`}>
                        ...
                        <div className="font-mono">
                          {formatPrice(collection.sellSignal.price)}
                        </div>
                      </div>
                    ) : (
                      <span className="text-gray-400">-</span>
...
export default TradingCollectionsTable3and6;
