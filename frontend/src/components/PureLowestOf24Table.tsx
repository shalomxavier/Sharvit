...
interface PureLowestOf24Collection {
  id: string;
...
const PureLowestOf24Table: React.FC<PureLowestOf24TableProps> = ({
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      q,
      (querySnapshot) => {
        const collectionsData: PureLowestOf24Collection[] = [];
        querySnapshot.forEach((doc) => {
          collectionsData.push({ id: doc.id, ...doc.data() } as PureLowestOf24Collection);
        });
      ...
      (err) => {
        console.error('Error fetching pure lowest of 24 collections:', err);
          ...
          <table className="min-w-full table-auto">
            <thead>
              <tr className="border-b">
                ...
                <th className="text-left py-2 px-3 font-semibold text-sm">Rule Met</th>
...
export default PureLowestOf24Table;
